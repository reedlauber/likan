...
function query_done(execute, model, query, first, options, callback, count) {
	...
	execute(sql_str, query.params, function(results) {
		if(results) {
			...
			if(query.process) {
				model._process(results, options, function(processed) {
...
function query(execute, model, columns, options, quick_callback) {
	...
	self.commit = function(callback) {
		if(self.include_count) {
			self.count(function(count) {
				query_done(execute, model, query, false, options, callback, count);
			});
		} else {
			query_done(execute, model, query, false, options, callback, undefined);
		}
	};
	...
	self.first = function(callback) {
		query_done(execute, model, query, true, options, callback, undefined);
	};
